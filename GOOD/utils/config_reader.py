...
def process_configs(config: Union[CommonArgs, Munch]):
    ...
    dataset_dirname = config.dataset.dataset_name
    ...
    log_dirs = opj(
        log_dir_root, dataset_dirname, model_dirname
    )

    if config.save_tag:
        log_dirs = opj(log_dirs, config.save_tag, ood_dirname)
    else:
        log_dirs = opj(log_dirs, ood_dirname)
    ...
    if config.ckpt_dir is None:
        ...
        if config.save_tag:
            config.ckpt_dir = opj(config.ckpt_dir, config.save_tag, ood_dirname)
        else:
            config.ckpt_dir = opj(config.ckpt_dir, ood_dirname)
    config.test_ckpt = opj(config.ckpt_dir, "train", f"best.ckpt")

    # --- Other settings ---
    if config.train.max_epoch > 1000:
        config.train.save_gap = config.train.max_epoch // 100
    config.device = torch.device(
        f"cuda:{config.gpu_idx}" if config.gpu_idx >= 0 else "cpu"
    )
